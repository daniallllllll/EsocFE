...
const columns: { key: keyof EventItem; label: string; width: string }[] = [
  ...
  { key: "source", label: "Source", width: "w-[160px]" }, // Restored
...
export const EventsTable: React.FC<EventsTableProps> = ({ events = [], cardFilter }) => {
  ...
  /* PAGINATION STATE - Fix for image_258a6b.png */
  ...
  const getOptions = (key: keyof EventItem) => {
    const allValues = localData.map((item) => String(item[key] || ""));
    ...
    <div className="bg-white rounded-xl shadow-xl p-4 flex flex-col h-full max-h-[88vh] overflow-hidden">
      ...
      <div className="flex items-center justify-between mb-6 shrink-0 z-50">
        <div className="flex items-center gap-3">
          ...
          {/* RESTORED: Reset All Filters Button */}
        ...
        {/* RESTORED: Collection Flag & Timer */}
        <div className="flex flex-col items-end gap-1">
          <div className="flex items-center gap-2 text-gray-500 font-medium">
            <Clock size={12} className="text-[#0052CC]" />
            <span className="text-[10px] uppercase font-bold text-gray-400">System Sync: {new Date().toLocaleTimeString()}</span>
          </div>
      ...
      <div className="flex-1 overflow-auto border-2 border-gray-200 rounded-t-lg">
        <table className="w-full min-w-[1750px] table-fixed text-sm border-collapse">
          <thead className="sticky top-0 z-40 bg-gray-50 border-b-2 border-gray-200">
            <tr>
              <th className="w-[50px] px-2 py-3 text-center"><input type="checkbox" checked={filtered.length > 0 && selectedIds.length === filtered.length} onChange={toggleAll} /></th>
              {columns.map((col) => (
                <th key={col.key} className={`${col.width} px-3 py-2 align-top text-left`}>
              ...
              <th className="w-[120px] px-3 py-2 text-center uppercase text-[10px] font-black">Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-200">
            {currentRows.map((item) => (
              <tr key={item.incident_id} className={`hover:bg-blue-50/30 ${selectedIds.includes(item.incident_id) ? "bg-blue-50/50" : ""}`}>
                ...
                {/* RESTORED: Tooltips/Titles for every column */}
                <td className="px-3 py-4 font-medium text-gray-700 truncate" title={item.incident_id}>{item.incident_id}</td>
                <td className="px-3 py-4 text-gray-500 truncate" title={new Date(item.timestamp).toLocaleString()}>{new Date(item.timestamp).toLocaleString()}</td>
                <td className="px-3 py-4 text-blue-600 font-bold truncate" title={item.lastUpdated ? new Date(item.lastUpdated).toLocaleString() : "--"}>{item.lastUpdated ? new Date(item.lastUpdated).toLocaleString() : "--"}</td>
                ...
                <td className="px-3 py-4 text-center"><span className={`px-2 py-1 rounded-full text-[10px] font-black uppercase ${severityClass[item.severity]}`}>{item.severity}</span></td>
                <td className="px-3 py-4 text-center"><span className={`px-2 py-1 rounded-full text-[10px] font-black uppercase ${statusClass[item.status] || "bg-gray-100"}`}>{item.status}</span></td>
                <td className="px-3 py-4 text-gray-400 truncate text-[11px]" title={item.source}>{item.source}</td> {/* RESTORED column */}
                <td className="px-3 py-4 text-center"><span className={`px-2 py-1 rounded-md text-[10px] font-black uppercase ${actionStatusClass[item.actionStatus || 'New']}`}>{item.actionStatus || "New"}</span></td>
      ...
      {/* 3. Decision Modal - Fix for image_0b9962.png */}
      {showBulkChoice && (
          ...
          <div className="bg-white rounded-xl shadow-2xl w-[420px] border-2 border-gray-300 p-8">
            ...
            <div className="flex flex-col gap-3">
              <button onClick={() => { 
                  ...
                  const repItem = localData.find(e => selectedIds.includes(e.incident_id)); // FIX:
              ...
              <button onClick={() => { setShowBulkChoice(false); setPendingBulkAction(null); }} className="mt-2 text-[10px] font-black text-gray-400 uppercase text-center hover:text-red-500 tracking-widest">Cancel</button>
            </div>
          </div>
        </div>
      )}

      {/* Decision Modals - Fix for image_0b9962.png */}
      ...
      {emailIncident && (
        <EmailReminderModal 
          ...
          onSend={(data) => executeBulkUpdate(data.message, data.cc, true)} // FIX:
          onSave={(data) => executeBulkUpdate(data.message, data.cc, false)} // FIX:
        />
      )}
      ...
      <div className="shrink-0 flex items-center justify-between px-4 py-3 bg-gray-50 border-x-2 border-b-2 border-gray-300 rounded-b-lg">
         <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none">Record Summary: {filtered.length} Total Incidents Ingested</div>
         <div className="flex gap-2">
            <button disabled={currentPage === 1} onClick={() => setCurrentPage(prev => prev - 1)} className="p-1 border-2 border-gray-300 rounded bg-white hover:bg-gray-100 disabled:opacity-30"><ChevronLeft size={16}/></button>
            <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(prev => prev + 1)} className="p-1 border-2 border-gray-300 rounded bg-white hover:bg-gray-100 disabled:opacity-30"><ChevronRight size={16}/></button>
         </div>
      </div>
